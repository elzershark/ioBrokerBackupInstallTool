...
echo.
echo   !!! Der USB-Stick muss nach jedem entfernen vom Raspy neu gemountet werden [4] !!!
...
echo   -----------------------------------------------------------------
echo			[3] USB Mount Kontrolle
echo			[4] USB Aktivieren
echo			[5] USB Auswerfen
echo			[6] USB Backup erstellen
echo			[7] Mit USB Backup ioBroker zuruecksetzen
echo			[8] Beenden
echo.
set asw=0
...
echo.
echo.
echo crontab -l ^| sed -e '/sudo mount -t/d' ^| crontab; ^(crontab ^-l ^&^& echo "@reboot sudo mount -t %vat% -o utf8,uid=iobroker,gid=iobroker,noatime /dev/%num% /media/usbstick") ^| crontab -; sudo mount -t %vat% -o utf8,uid=iobroker,gid=iobroker,noatime /dev/%num% /media/usbstick>Programm/usb.txt
